...
module private ArrNodeImplementationAggregate =
...
module AArr =
    ...
    module Readers =
        type MapReader<'T1, 'T2>(reader : IArrayReader<'T1>, mapping : 'T1 -> 'T2) =
        ...
        type ChooseReader<'a, 'b>(input : IArrayReader<'a>, mapping : 'a -> option<'b>) =
            inherit AbstractReader<arrdelta<'b>>(ArrDelta.empty)
            ...
            static let aggregate =
                {
                    ArrNodeImplementationAggregate.Zero = 0
                    ArrNodeImplementationAggregate.Add = OptimizedClosures.FSharpFunc<_,_,_>.Adapt((+))
                    ArrNodeImplementationAggregate.View = function Some _ -> 1 | None -> 0
                }
            ...
            override x.Compute(t : AdaptiveToken) =
                let ops = input.GetChanges t
                let ops = ops |> ArrDelta.map mapping
                ...
                for o in ops do
                    let offset = 
                        ...
                        | Some offset -> offset
                        | None -> 0
                    let mutable srcIndex = o.Index
                    let mutable dstIndex = offset
                    let offset = ()
                    ...
                    for r in 1 .. o.Count do
                        ...
                        | Some (rest, removed) ->
                            ...
                            match removed with
                            | Some _ -> 
                                emit { Index = dstIndex; Count = 1; Elements = Arr.empty }
                            | None ->
                                ()
                        | None ->
                            ()
                    ...
                    for e in o.Elements do
                        prefix <- prefix.InsertAt(srcIndex, e)
                        match e with
                        | Some e ->
                            emit { Index = dstIndex; Count = 0; Elements = Arr.single e }
                            dstIndex <- dstIndex + 1
                        | None ->
                            ()
                        srcIndex <- srcIndex + 1
    ...
    let force (array : aarr<'a>) =
        array.Content |> AVal.force
    ...
    let map (mapping : 'T1 -> 'T2) (input : aarr<'T1>) : aarr<'T2> =
        if input.IsConstant then
            ConstantArray(Lazy<arr<'T2>>(fun () -> force input |> Arr.map mapping)) :> aarr<_>
        else
            ofReader <| fun () -> Readers.MapReader(input.GetReader(), mapping)
            
    /// Adaptively applies the given mapping function to all elements and returns a new aarr containing the results.
    let choose (mapping : 'T1 -> option<'T2>) (input : aarr<'T1>) : aarr<'T2> =
        if input.IsConstant then
            ConstantArray(Lazy<arr<'T2>>(fun () -> force input |> Arr.choose mapping)) :> aarr<_>
        else
            ofReader <| fun () ->
                Readers.ChooseReader(input.GetReader(), mapping)
            
    let collect (mapping : 'T1 -> aarr<'T2>) (input : aarr<'T1>) : aarr<'T2> =
        if input.IsConstant then
            let arrs = input |> force |> Arr.map mapping
            if arrs |> Arr.forall (fun a -> a.IsConstant) then
                ConstantArray(Lazy<arr<'T2>>(fun () -> arrs |> Arr.collect force)) :> aarr<_>
            else
                // TODO: better implementation?
                ofReader <| fun () ->
                    let set = ofArr arrs
                    Readers.CollectReader(set.GetReader(), id)
                
        else
            ofReader <| fun () ->
                Readers.CollectReader(input.GetReader(), mapping)
